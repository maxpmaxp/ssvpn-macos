...
- (id)performDefaultImplementation
{
    ...
    if (  connection  ) {
        if (  ! [connection isDisconnected]  ) {
            [connection addToLog: @"*SurfSafeVPN: Disconnecting; AppleScript 'disconnect' invoked"];
...
- (id)performDefaultImplementation
{
    ...
    while (  connection = [connEnum nextObject]  ) {
        if (  ! [connection isDisconnected]  ) {
            [connection addToLog: @"*SurfSafeVPN: Disconnecting; AppleScript 'disconnect all' invoked"];
            [connection disconnectAndWait: [NSNumber numberWithBool: NO] userKnows: YES];
            nDisconnecting++;
        }
    }
    ...
    while (  connection = [connEnum nextObject]  ) {
        if (  ! [connection isDisconnected]  ) {
            ...
            if (  ! (   [gTbDefaults boolForKey: autoConnectkey]
                ...
                [connection addToLog:@"*SurfSafeVPN: Disconnecting; AppleScript 'disconnect all except when computer starts' invoked"];
